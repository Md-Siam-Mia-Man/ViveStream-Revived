...
*Note: A single video can have multiple artists (e.g., "Feat. X & Y").*
...
3.  Checks if specific columns exist (`hasColumn`) to support upgrades from older versions.
    *   *Example*: When `subtitlePath` was added, a check ensures existing databases get the new column without data loss.
